    ...
    class Collector
    {
        ...
        public static void creNfo(Av av) {
            ...
            if (mat.Success){
               ...
               mat = new Regex(@"<p><span class=""header"">長度:</span>\s*(\d*)分鐘</p>
").Match(av.Html);
            if (mat.Success)
            {
            ...
            mat = new Regex(@"<p><span class=""header"">製作商:</span> <a href="".*?"">(.*?)</a>
").Match(av.Html);
            if (mat.Success) 
            {
            ...
            MatchCollection tags = new Regex(@"<span class=""genre""><a href="".*?"">(.*?)</a></span>
").Matches(av.Html);
